...
import com.game.smallgiant1010.HelpMethods;
import gamestates.GameState;
import gamestates.Playing;

import com.game.smallgiant1010.Game;
import com.game.smallgiant1010.LoadSave;
import lightning_mage.Constants.PlayerConstants;
...
import static com.game.smallgiant1010.HelpMethods.*;
...
public class WASDPlayer extends Entity {
    ...
    private void durationLeft(){
        if((playerAction == samurai.Constants.PlayerConstants.ATTACK_1 || playerAction == PlayerConstants.ATTACK_2) && BlockDuration == 1000 && dwindle) {
            regen = false;
            currentDuration -= 10;
...
//    }
    ...
    private void updateAnimationTick() {
        ...
        if (aniTick >= aniSpeed) {
            aniTick = 0;
            aniIndex++;
            if (aniIndex >= PlayerConstants.GetSpriteAmount(playerAction) && alive) {
                aniIndex = 0;
                attacking1 = false;
                attacking2 = false;
                lightningBall = false;
    ...
    private void setAnimations() {
        ...
        if (inAir && alive) {
            if (airSpeed > 50) {
                ...
                aniSpeed = 50;
                playerAction = PlayerConstants.FALL;
            }
        }
        if (hurt && alive){
            moving = false;
        ...
        if (attacking1 && !attacking2 && !lightningBall && !hurt && alive) {
            ...
            aniSpeed = 10;
            playerAction = PlayerConstants.LIGHT_BALL;
        }
        else if(attacking2 && !attacking1 && !lightningBall && !hurt && alive) {
            aniSpeed = 25;
            playerAction = PlayerConstants.ATTACK_2;
        }
        if (!alive) {
            moving = false;
            aniSpeed = 120;
            playerAction = samurai.Constants.PlayerConstants.DEAD;
        }
    ...
    private void updatePos(){
        ...
        if (inAir) {
            if (HelpMethods.canMoveHere(hitBox.x, hitBox.y + airSpeed, hitBox.width, hitBox.height)){
                hitBox.y += airSpeed;
                airSpeed += gravity;
                updateXPos(xSpeed);
            }
            else {
                hitBox.y = HelpMethods.GetEntityYPosWithinFrame(hitBox, airSpeed);
                if (airSpeed > 0){
                    resetInAir();
                }
                else {
        ...
        if(!inAir){
            if(HelpMethods.EntityOnFloor(hitBox)){
                inAir = true;
            }
        }
    ...
    private void updateXPos(float xSpeed) {
        if (HelpMethods.canMoveHere(hitBox.x+xSpeed, hitBox.y, hitBox.width, hitBox.height)){
            hitBox.x += xSpeed;
        }
        else {
    ...
    public void setLightningBall(boolean lightningBall){
        if (!attacking2 && !attacking1 && !currentlyAttacking) {
            ...
            for (int i = aniIndex; i < PlayerConstants.GetSpriteAmount(playerAction); i++) {
